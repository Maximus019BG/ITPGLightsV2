import React, { useState, useEffect, useRef, useMemo } from 'react';

interface SideBarProps {
  setFilters: (filters: string[]) => void;
  setSearchTerm: (searchTerm: string) => void;
  setActiveFilters: (activeFilters: string[]) => void;
}

const SideBar: React.FC<SideBarProps> = ({ setFilters, setSearchTerm, setActiveFilters }) => {
  const [isSpanClicked, setIsSpanClicked] = useState<boolean[]>([false, false, false]);
  const [isDarkMode, setIsDarkMode] = useState<boolean>(false);  
  const [isSidebarOpen, setIsSidebarOpen] = useState(true); 
  const checkboxRef = useRef(null);
  const [activeFilters, setActiveFiltersLocal] = useState<string[]>([]);

  const filters = useMemo(() => ['Концерт', 'Фестивал', 'Кино', 'Театър', 'Състезание', 'Опера','Спорт'], []); 

  useEffect(() => {
    if (typeof window !== 'undefined') {
      setIsDarkMode(localStorage.theme === 'dark' || (!('theme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches));
    }
  }, []);

  useEffect(() => {
    if (isDarkMode) {
      document.documentElement.classList.add('dark');
    } else {
      document.documentElement.classList.remove('dark');
    }
    // Save the dark mode
    localStorage.setItem('theme', isDarkMode ? 'dark' : 'light');
    console.log(localStorage.getItem('theme'));
  }, [isDarkMode]);

  const handleDarkMode = () => {
    setIsDarkMode(!isDarkMode);
    if (checkboxRef.current) {
      (checkboxRef.current as HTMLInputElement).click();
    }
  };

  const handleSpanClick = (index: number) => {
    const updatedIsSpanClicked = [...isSpanClicked];
    updatedIsSpanClicked[index] = !updatedIsSpanClicked[index];
    setIsSpanClicked(updatedIsSpanClicked);
    
    if (updatedIsSpanClicked[index]) {
      setActiveFiltersLocal([...activeFilters, filters[index]]);
    } else {
      setActiveFiltersLocal(activeFilters.filter(filter => filter !== filters[index]));
    }
  };

  useEffect(() => {
    setFilters(filters);
    setActiveFilters(activeFilters);
  }, [filters, setFilters, activeFilters, setActiveFilters]);

  return (
    <div className='z-10 xl:-mt-11 lg:-mt-11 md:-mt-11 '>
      <button className="toggle-button" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
      <span className={isSidebarOpen ? 'button-text button-text-show1' : 'button-text button-text-hide'}>Затвори</span>
      <span className={!isSidebarOpen ? 'button-text button-text-show ' : 'button-text button-text-hide'}>Отвори</span>
      </button>
      <div className={`sidebar ${isSidebarOpen ? 'open' : 'closed'} flex flex-col shadow dark:shadow-none shadow-slate-400 items-start mt-[48px] fixed h-screen z-6 w-72 text-lg  pt-4 bg-white dark:bg-gray-800`}>
        <h1 className={`mb-4 ml-5 text-2xl font-semibold text-gray-900 dark:text-white`}>Филтри</h1>

        {filters.map((filter, index) => (
          
          <span
            key={index}
            className={`border ${isSpanClicked[index] ? 'border-green-500 font-medium text-green-600 dark:text-green-400 dark:bg-[#011E2B] bg-green-100' : 'border-white font-normal'} px-5 font py-2 rounded-full cursor-pointer m-3 relative unselectable text-gray-900 dark:text-white`}
            onClick={() => handleSpanClick(index)}
          >
            {filter}
            {isSpanClicked[index] && (
              <span className="p-2 px-2 text-lg font-bold text-green-600 cursor-pointer" onClick={() => handleSpanClick(index)}>
                &nbsp; x
              </span>
            )}
          </span>
        ))}
        <div className='relative bottom-20 left-8'>
        <button onClick={handleDarkMode} className="absolute text-gray-900 dark:text-white top-20 ">
    {isDarkMode ? (
      <div className='flex -ml-3'>
        <svg className="w-10 h-10 fill-current swap-on" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z"/></svg>
        <h1 className='w-40 mt-1 font-semibold dark:text-white'>Светла тема </h1>
    </div>
    ): (
      <div className='flex -ml-3'>
        <svg className="w-10 h-10 fill-current swap-off" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z"/></svg>
        <h1 className='w-40 mt-1 font-semibold text-black'>Тъмна тема </h1>
      </div>
    )}
          </button>
        </div>
      </div>
    </div>
  );
};

export default SideBar;